...
const $=s=>document.querySelector(s), home=$('#scene'), modalHost=$('#modal-scene'), dialog=$('#success');
const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;
const records=new Map();let lastAction='签到',bowStart=-100,renderer,scene,camera,rig,upper,head,petals=[],canvasParent=home;
function tick(){const d=new Date();$('#clock-time').textContent=d.toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit',hour12:false});$('#clock-seconds').textContent=String(d.getSeconds()).padStart(2,'0');$('#date').textContent=d.toLocaleDateString('zh-CN',{month:'long',day:'numeric',weekday:'long'});}tick();setInterval(tick,1000);
function refresh(){const r=records.get($('#employee').value)||{};$('#avatar').textContent=$('#employee').value[0];$('#status').textContent=r.out?'今日已完成':r.in?'已签到':'待签到';$('#status').classList.toggle('done',!!r.in);$('#check').disabled=!!r.out;$('#check span').textContent=r.out?'今日打卡已完成':r.in?'签退下班':'签到上班';$('#feedback').textContent=r.out?`已签退 · ${r.out}`:r.in?`已签到 · ${r.in}`:'今天也加油！';}
$('#employee').addEventListener('change',refresh);
...
function openGreeting(preview=false){$('#success-title').textContent=preview?'你好呀！':lastAction+'成功！';const r=records.get($('#employee').value)||{};$('#success-detail').textContent=preview?'很高兴见到你 ♡':`${$('#employee').value} · ${lastAction==='签到'?r.in:r.out}`;$('.thanks').textContent=lastAction==='签退'&&!preview?'辛苦啦，明天见！':'今天也要元气满满哦！';$('#continue').firstChild.textContent='好哒 ';dialog.showModal();document.body.classList.add('modal-open');if(renderer){canvasParent=modalHost;modalHost.append(renderer.domElement);resize();}bowStart=performance.now()/1000;$('#continue').focus();}
function closeGreeting(){dialog.close();}dialog.addEventListener('close',()=>{document.body.classList.remove('modal-open');if(renderer){canvasParent=home;home.append(renderer.domElement);resize();}$('#check').disabled?$('#preview-bow').focus():$('#check').focus();});$('#close-modal').onclick=closeGreeting;$('#continue').onclick=closeGreeting;$('#replay').onclick=()=>{bowStart=performance.now()/1000};$('#preview-bow').onclick=()=>openGreeting(true);$('#check').onclick=()=>{const name=$('#employee').value,r=records.get(name)||{};const time=new Date().toLocaleTimeString('zh-CN',{hour12:false});if(!r.in){r.in=time;lastAction='签到';}else if(!r.out){r.out=time;lastAction='签退';}else return;records.set(name,r);refresh();openGreeting();};
...
}catch(error){console.error(error);home.innerHTML='<p class="loading">立体人物暂不可用，仍可体验打卡。</p>';modalHost.innerHTML='<p style="padding-top:90px;font-size:48px">✿</p>';}
