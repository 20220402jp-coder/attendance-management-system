...
已发布下载包为 **v1.0.2**，包含近期的个人码报班、扫码打卡和权限改动。源码启动需要 Python，下载的成品程序不需要。
...
- Windows（最常用）：打开「下载」文件夹，右键 `AttendanceSystem-...-Windows-x64.exe.zip`，选择「全部解压缩」，确认目标文件夹后点击「解压缩」。解压完成后打开新文件夹，双击里面的 `.exe` 程序。不要在压缩包预览窗口中双击，也不要只把 `.exe` 拖到桌面运行。
